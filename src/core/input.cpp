...
bool input_is_key_just_pressed(SDL_Scancode scancode) {
    return state.key_pressed_current[scancode] && !state.key_pressed_previous[scancode];
}
...
bool input_is_mouse_button_just_pressed(InputMouseButton button) {
    return state.mouse_pressed_current[button] && !state.mouse_pressed_previous[button];
}
